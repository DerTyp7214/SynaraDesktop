...
class WindowsSynaraTray : SynaraTray {
    ...
    override fun show(
        ...
        baseIcon = try {
            if (resource != null) {
                ...
                val coloredResource = Thread.currentThread().contextClassLoader.getResource("tray.png")
                    ?: this::class.java.classLoader.getResource("tray.png")
                
                if (coloredResource != null) {
                    ImageIO.read(coloredResource)
                } else {
                    val whiteResource = Thread.currentThread().contextClassLoader.getResource("tray-white.png")
                        ?: this::class.java.classLoader.getResource("tray-white.png")
                    if (whiteResource != null) ImageIO.read(whiteResource) else null
                }
            }
        } catch (_: Exception) {
            null
    ...
    override fun setBadge(color: Color?) {
        ...
        g2d.color = AwtColor(0, 0, 0, 120)
        g2d.fillOval(x - 1, -1, badgeSize + 2, badgeSize + 2)
        g2d.color = AwtColor(color.toArgb(), true)
